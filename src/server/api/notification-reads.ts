...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user?.sub) return jsonError('Unauthorized', 401);

    const url = new URL(request.url);
    const ids = parseIdsParam(url.searchParams.get('ids'));
    if (ids.length === 0) return NextResponse.json({ readIds: [] });
    ...
    return NextResponse.json({ readIds: rows.map((r: { notificationId: unknown }) => String(r.notificationId)) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ids.length === 0) return NextResponse.json({ ok: true, upserted: 0 });
